...
class TestLengthBasedSelection:
    def test_select_by_instruction_output(self):
        ...
        result = LengthBasedSelector().select(samples, k=2)
        assert len(result) == 2
    ...
    def test_select_k_zero(self):
        ...
        result = LengthBasedSelector().select(samples, k=0)
        assert result == []

    def test_select_empty(self):
        result = LengthBasedSelector().select([], k=3)
        assert result == []
    ...
    def test_select_missing_fields(self):
        ...
        result = LengthBasedSelector().select(samples, k=3)
        assert len(result) == 3
